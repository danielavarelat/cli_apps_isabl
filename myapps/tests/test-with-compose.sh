...
cd $APPS_DIR && py.test -vv --cov=myapps --cov-report=term-missing tests/ --commit
pylint myapps --rcfile=$APPS_DIR/.pylintrc
pydocstyle myapps --config=$APPS_DIR/.pydocstylerc
